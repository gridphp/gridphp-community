...
You can also set specific color for your labels. e.g. General label will get green badge.

	$col["badgeoptions"]["colors"] = array("General"=>"green","Home"=>"orange","Work"=>"blue","Other"=>"gray","Travel"=>"red");

Colors can be one of these options.

	'green','yellow','blue','red','purple','orange','sky','green-light','lime','teal','coral',
	'brown','olive','pink','mint','cyan','lavender','gold','salmon','indigo','peach','aqua',
	'rose','sand','tan','sea','plum','ice','honey'
...
Set Add and Edit form & View dialog width. This can be used with combination of css customization of dialog forms.
	...
	$opt["add_options"]["width"] = '600';
	$opt["edit_options"]["width"] = '600';
	$opt["view_options"]["width"] = '600';
...
To handle database errors or provide custom error messages, you can use the on_error event.
Like other events, The `on_error` takes 3 params (function-name, class-object or null-if-global-func, continue-default-grid-operation)

	$e["on_error"] = array("handle_error","",false);
	$g->set_events($e);

	function handle_error($param)
	{
		$msg = $param["msg"]; // error message
		$sql = $param["sql"]; // sql statement that failed
		$data = $param["data"]; // data in case of prepared sql
		
		// perform custom logging or display
		phpgrid_error("Custom Error: " . $msg);
	}
...
To run a custom code before submit, use beforeSubmit with `add_options` or `edit_options` for add/edit dialog.
...
### Q) How to show horizontal scroll bar on top of the grid data?

You need to set following params:

	$opt["loadComplete"] = 'function(){
		jQuery(".ui-jqgrid-hdiv").css("overflow","auto");
		jQuery(".ui-jqgrid-hdiv").scroll(function(){
			jQuery(".ui-jqgrid-bdiv").scrollLeft(jQuery(".ui-jqgrid-hdiv").scrollLeft());
		});
	}';
	$g->set_options($grid);

### Q) How to increase height or width of scrollbar?

You can try these css, change 15px to desired value.
...
### Q) How to decrease grid height less than the fixed value (e.g. 500px)?
...
In html code:
	...
	<style>
	/* paging and record count on top pager, set display:none to hide */
...
### Q) How to change background color of 'select2' for different dropdowns?

Use following on page css.
