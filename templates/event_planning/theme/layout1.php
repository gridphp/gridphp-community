...
$links = array();
$links["dashboard"] = array("Dashboard","index.php?rep=dashboard",false);
$links["events"] = array("Events","index.php?mod=events",false);
$links["speakers"] = array("Speakers","index.php?mod=speakers",false);
$links["event_tasks"] = array("Event Tasks","index.php?mod=event_tasks",false);
$links["attendees"] = array("Attendees","index.php?mod=attendees",false);
$links["venues"] = array("Venues","index.php?mod=venues",false);
$links["logistics"] = array("Logistics","index.php?mod=logistics",false);
$links["marketing_promotions"] = array("Marketing Promotions","index.php?mod=marketing_promotions",false);
$links["post_event_evaluations"] = array("Post Event Evaluations","index.php?mod=post_event_evaluations",false);
$links["users"] = array("Users","index.php?mod=users",true);
$links["settings"] = array("Settings","index.php?mod=settings",true);

$reports = array();
$reports["event_details"] = array("Event Details","index.php?rep=event_details",false);
$reports["attendee_information"] = array("Attendee Information","index.php?rep=attendee_information",false);
$reports["task_assignments"] = array("Task Assignments","index.php?rep=task_assignments",false);
...
<head>
	...
	<style>:root{--bs-dark-rgb: 107, 82, 147;--tabcolor: #63498d;--tabhover: #543E78;}</style>
</head>
...
<body class="d-flex flex-column h-100">
	<header>
		...
		<nav class="navbar navbar-expand-md navbar-dark fixed-top bg-dark">
			<div class="container-fluid">
				<div class="container-fluid">
					<div class="row">
						<div class="col-6 col-sm-9" style="display:flex;align-items:center;gap:10px">
							<div class="logo">
								<?php
								$words = array_filter(explode(' ', trim(APP_NAME)));
								echo count($words) >= 2 
									? strtoupper($words[0][0] . $words[1][0])
									: strtoupper(substr(APP_NAME, 0, 2));
								?>
							</div>
							<a class="navbar-brand" style="font-size:x-large" href="index.php"><?php echo APP_NAME ?></a>
							<span class="d-none d-sm-inline" style="font-size:11px;color:white;margin-top: 9px;opacity: 0.5;">Made with <a style="text-decoration:none;color:white" href="https://www.gridphp.com/">Grid 4 PHP Framework</a></span> 
							<span class="d-none d-sm-inline" style="font-size:10px;color:transparent;margin-top: 9px;">v1.0.0 - 30/10/2025</span>
						</div>
						<div class="col-6 col-sm-3 d-flex justify-content-end align-items-center">
							<?php if (empty($_GET["embed"]))  { ?>
							<a target="_parent" href="/download/" class="btn btn-warning btn" style="margin-right:20px; padding: 5px 10px;">Download Now</a>
							<?php } ?>
	...
	<main class="flex-shrink-0">
		...
		<div class="container-fluid">
			<ul class="nav nav-tabs d-none d-sm-flex fixed-top" id="myTab" role="tablist">
				<?php
				if (!empty($links))
				{
					...
					{	
						...
						$class = "";
						// select url passed mod or dashboard (rep)
						if (get_clean($m) == strtolower($mod) || get_clean($m) == strtolower($rep))
							$class = "active";
						else if ($hidden)
							$class = "hide";
					?>
					...
					// Display Reports dropdown if there are any reports
					if (!empty($reports))
					{
						$class_dropdown = "";
						// select reports for any
						if (!empty($rep) && strtolower($rep) != "dashboard")
							$class_dropdown = "active";
						?>
						<li class="nav-item dropdown" role="presentation">
							<a id="reportDropdown" class="nav-link dropdown-toggle <?php echo $class_dropdown ?>" data-bs-toggle="dropdown" href="#" role="button" aria-expanded="false">Reports</a>
							<div class="dropdown-menu" aria-labelledby="reportDropdown">
								...
								foreach($reports as $v) 
								{
									...
									$class = "";
									if (get_clean($m) == strtolower($rep))									
										$class = "active";
									else if ($hidden)
										$class = "hide";
								?>
								<a class="dropdown-item <?php echo $class ?>" href="<?php echo $url?>"><?php echo $m?></a>
								<?php 
								} 
								?>
							</div>
			...
			<?php
				// merge in dropdown for sm screen 
				$links = array_merge($links,$reports);
				if (!empty($links))
				{
					foreach($links as $k=>$v)
					{
	...
	<link rel="stylesheet" href="<?php echo PHPGRID_URL?>lib/js/integration/jodit/3.7.2/jodit.min.css">
	<script src="<?php echo PHPGRID_URL?>lib/js/integration/jodit/3.7.2/jodit.min.js"></script>
	...
	function gridLoad()
	{
		...
		$(document).click(function(e){
			if($(e.target).parent().hasClass('img-gridcell'))
			{
				...
				a.addClass("fancybox");
				jQuery.fancybox.open({"autoScale":true,"content":$("<img width='100%' id='x'>").attr("src",$(a).attr('href')).prop('outerHTML')});
			}
		});
	}
	...
	jQuery("document").ready(function(){
		setTimeout(()=>{
			...
			// if no grid on page, skip
			if (!jQuery('.ui-jqgrid').length)
			{
				// for dashboard screen
				jQuery(".tab-content > .tab-pane").css("height","inherit");
				return;
			}
	...
	<script>
	// Smooth Tab Switching with Page Reload for Grid 4 PHP
	(function() {
		// Set active tab on page load based on current URL
		document.addEventListener('DOMContentLoaded', function() {
			const currentUrl = window.location.href;
			const tabLinks = document.querySelectorAll('.nav-tabs .nav-link');
			
			// Mark active tab based on href match
			...
			tabLinks.forEach(link => {
				link.addEventListener('click', function(e) {
					...
					// Visual feedback - update active state immediately
					...
					// Short delay for smooth transition, then navigate
					setTimeout(function() {
		...
		function showLoadingOverlay() {
			...
			if (!overlay) {
				...
				spinner.style.cssText = `
					...
					border-top: 4px solid var(--tabcolor, #2196F3);
					border-radius: 50%;
