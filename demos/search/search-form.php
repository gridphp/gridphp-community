...
$g->set_actions(array(	
						...
						"search" => "group", // show single/multi field search condition (e.g. simple or advance)
...
<body>
	<div style="margin:10px">
		<fieldset style="font-family:tahoma; font-size:12px">
			...
			<form action="" style="display:flex; width:100%; gap:30px; align-items:center; justify-content: space-between; flex-grow:1">
				<div>
					Search Name: <input type="text" id="filter"/>
				</div>
				<div>
					Templates:
					<select id="search_tpl">
						<option value="">- Search Templates -</option>
				...
				<div>
					Select Name: <select id="search_name" multiple="multiple">
    ...
    jQuery("input[name=clear]").click(function() {
		...
		let grid = jQuery("#list1");

		// Set retaining filter
		grid.retainFilters("clear");
	...
	let filterData = function() {
        ...
        var searchFilter = jQuery("#filter").val();
		...
		// get existing toolbar filters
		var toolbar_filters = {};
		if (grid[0].p.postData.filters)
			toolbar_filters = JSON.parse(grid[0].p.postData.filters);
		
		// Set retaining filter
		grid.retainFilters(f);

		// set existing toolbar filters
		jQuery.extend(grid[0].p.postData,{filters:JSON.stringify(toolbar_filters)});
    ...
    jQuery("#search_tpl").change(filterData);
    jQuery("input[name=filter]").click(filterData);
