...
class TCPDF_EasyTable extends TCPDF{
    ...
    */
    ...
    private function _ProcessMultiCellImage($pdfImage, $cellWidth, $cellHeight){
        ...
        // echo $pdfImage->GetImageHeight();
        // echo "...".$cellHeight;die;

        // $pdfImage->ScaleWidthTo($cellWidth);
        // $pdfImage->ScaleHeightTo(75);
        ...
        if($pdfImage->Exists()){
            ...
            $this->Image($pdfImage->GetImagePath(), $imageX, $imageY, $pdfImage->GetImageWidth(), $pdfImage->GetImageHeight(), $pdfImage->GetImageFileType(),'','',false,300,'',false,false,0,true);
    ...
    public function Header() {
        ...
        // Image($file, $x='', $y='', $w=0, $h=0, $type='', $link='', $align='', $resize=false, $dpi=300, $palign='', $ismask=false, $imgmask=false, $border=0, $fitbox=false, $hidden=false, $fitonpage=false, $alt=false, $altimgs=array())
        $this->Image($image_file, PDF_MARGIN_LEFT, PDF_MARGIN_TOP+5, 50, '', 'JPG', '', 'T', false, 300, '', false, false, 0, false, false, false);
        $this->SetTopMargin(PDF_MARGIN_TOP+18);
    }
